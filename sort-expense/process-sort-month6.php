<div class="container mt-1">
    ...
    <table class="table table-width table-hover table-striped bg-light caption-top my-2 mt-3">
        <caption class="top text-dark display-6 text-center">Expenses Report Form (6 Months)</caption>
        ...
        <tbody>
            ...
            $sqlMonth6 = "SELECT * FROM expenses INNER JOIN registration ON expenses.user_id = registration.user_id WHERE expenses.user_id = '$activeUser' AND expenses.date_created BETWEEN DATE_SUB( CURDATE(), INTERVAL 6 MONTH) AND CURDATE()";
            $month6Result = mysqli_query($conn, $sqlMonth6);

            if (mysqli_num_rows($month6Result) > 0) {
                $expenseIndex = 1;
                $sumExpenses = 0;
                while ($rowMonth6 = mysqli_fetch_assoc($month6Result)) {
                    $sumExpenses += $rowMonth6["amount"];
            ?>

            <tr>
                <th scope="row"><?php echo $expenseIndex++; ?></th>
                <td><?php echo $rowMonth6["item"]; ?></td>
                <td><?php echo $rowMonth6["amount"]; ?></td>
                <td><?php echo $rowMonth6["date_created"]; ?></td>
                <td><?php echo $rowMonth6["expense_description"]; ?></td>
                <td colspan="2" class="not-print d-flex">
                    <a href="view-expense.php?view_expense=<?php echo $rowMonth6["expense_id"]; ?>" title="View Expense"
                        class="d-flex mx-1 btn btn-info text-light">
                        <i class="fas fa-eye me-1"></i><span class="action-btn">View</span></a>
                    <a href="edit-expense.php?edit_expense=<?php echo $rowMonth6["expense_id"]; ?>"
                    ...
                    <a href="expenditures.php?delete_expense=<?php echo $rowMonth6["expense_id"]; ?>"
            ...
            <div class="alert alert-info">
                <span>No record for 6 months.</span>
