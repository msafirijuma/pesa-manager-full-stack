<div class="container mt-1">
    ...
    <table class="table table-width table-hover table-striped bg-light caption-top my-2 mt-3">
        <caption class="top text-dark display-6 text-center">Expenses Report Form Yesterday (
            <?php
        ...
        <tbody>
            ...
            $sqlYesterday = "SELECT * FROM expenses INNER JOIN registration ON expenses.user_id = registration.user_id WHERE expenses.user_id = '$activeUser' AND expenses.date_created BETWEEN DATE_SUB( CURDATE(), INTERVAL 1 DAY) AND CURDATE()";
            ...
            if (mysqli_num_rows($yesterdayResult) > 0) {
                ...
                while ($rowYesterday = mysqli_fetch_assoc($yesterdayResult)) {
                    $sumExpenses += $rowYesterday["amount"];
            ?>
            ...
            <tr>
                ...
                <td><?php echo $rowYesterday["expense_description"]; ?></td>
                <td colspan="2" class="not-print d-flex">
                    <a href="view-expense.php?view_expense=<?php echo $rowYesterday["expense_id"]; ?>"
                        title="View Expense" class="d-flex mx-1 btn btn-info text-light">
                        <i class="fas fa-eye me-1"></i><span class="action-btn">View</span></a>
                    <a href="edit-expense.php?edit_expense=<?php echo $rowYesterday["expense_id"]; ?>"
                    ...
                    <a href="expenditures.php?delete_expense=<?php echo $rowYesterday["expense_id"]; ?>"
